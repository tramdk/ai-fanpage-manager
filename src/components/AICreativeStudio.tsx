...
export const AICreativeStudio: React.FC<AICreativeStudioProps> = memo(({ post, api, onSave, onClose, title = "AI Creative Studio" }) => {
  ...
  const handleRegenText = useCallback(async () => {
    ...
    try {
      ...
      console.error("AI Text Regen Failed", err);
    } finally {
  ...
  return (
    <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-md flex items-center justify-center z-[200] p-4">
      <div className="bg-slate-900 border border-slate-800 rounded-[32px] shadow-3xl w-full max-w-4xl max-h-[90vh] flex flex-col animate-in zoom-in-95 duration-200 overflow-hidden relative">
        <div className="absolute top-0 right-0 w-64 h-64 bg-emerald-500/5 blur-[100px] -mr-32 -mt-32"></div>

        {/* Header */}
        <div className="px-8 py-6 border-b border-slate-800 flex justify-between items-center bg-slate-900/50 relative z-10">
          <div className="flex items-center space-x-4">
            <div className="p-2.5 bg-emerald-500 text-white rounded-xl shadow-lg shadow-emerald-500/10"><Bot size={24} /></div>
            <div>
               <h3 className="text-lg font-bold text-slate-50 uppercase tracking-tight leading-none">{title}</h3>
               <p className="text-[9px] font-black text-slate-600 uppercase tracking-[0.2em] mt-2">Strategic Node Engineering</p>
            </div>
          </div>
          <button onClick={onClose} className="p-2 text-slate-500 hover:text-slate-300 transition-all"><X size={24} /></button>
        </div>

        {/* Body */}
        <div className="flex-1 overflow-y-auto p-10 space-y-8 bg-slate-950/50 custom-scrollbar relative z-10">
          <div className="bg-slate-900 border border-slate-800 p-6 rounded-2xl text-slate-400 flex items-start shadow-sm border-l-4 border-l-emerald-500">
            <Info className="w-6 h-6 mr-4 flex-shrink-0 text-emerald-500" />
            <div className="flex-1">
              <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-1">{t('aiGeneration')}</p>
          ...
          <div className="space-y-6">
            <div className="flex justify-between items-end bg-slate-900/50 p-6 rounded-t-2xl border border-slate-800 shadow-sm">
              <div className="space-y-1">
                <label className="text-[8px] font-black text-slate-600 uppercase tracking-widest block mb-1 ml-1">Context Category</label>
              ...
              <div className="flex gap-3">
                <button onClick={() => setShowAdvanced(prev => !prev)} className={`px-5 py-2.5 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all flex items-center border ${showAdvanced ? 'bg-indigo-500 border-indigo-500 text-white shadow-xl' : 'bg-slate-800 text-slate-400 border-slate-700 hover:text-slate-200'}`}>
            ...
            {showAdvanced ? (
              <div className="p-6 bg-slate-900 border-x border-slate-800 animate-in slide-in-from-top-2 duration-300">
                <label className="block text-[8px] font-black text-indigo-400 uppercase tracking-widest mb-3 ml-1">Context Injections</label>
                <textarea 
                  className="w-full bg-slate-950 border border-slate-800 rounded-xl p-5 text-xs font-bold text-slate-200 outline-none focus:border-emerald-500/50 transition-all shadow-inner resize-none" 
                  rows={3} 
                  value={prompt} 
            ...
            <textarea 
              className="w-full border border-slate-800 border-t-0 rounded-b-2xl p-8 text-sm font-bold leading-relaxed text-slate-300 focus:text-slate-100 outline-none bg-slate-900 shadow-inner min-h-[300px] resize-none transition-all" 
              value={content} 
          ...
          <div className="space-y-6">
            <div className="flex justify-between items-center bg-slate-900 border border-slate-800 p-5 px-8 rounded-2xl">
               <span className="text-[9px] font-black text-slate-600 uppercase tracking-widest">Visual Assets Pipeline</span>
            ...
            <div className="grid grid-cols-4 gap-6">
              {media.map(item => (
                <div key={item.id} className="relative aspect-square rounded-2xl overflow-hidden bg-slate-800 border border-slate-700 group hover:border-emerald-500/50 transition-all duration-500">
              ...
              {media.length === 0 && !isGeneratingImage ? (
                <div className="aspect-square bg-slate-900/50 rounded-2xl flex flex-col items-center justify-center border border-dashed border-slate-800 text-slate-800">
        ...
        {/* Footer */}
        <div className="px-8 py-6 border-t border-slate-800 flex justify-end items-center space-x-6 bg-slate-900/50 relative z-10">
           <button onClick={onClose} className="text-[10px] font-black uppercase text-slate-600 hover:text-slate-400 tracking-widest transition-colors">Discard changes</button>
           <button 
