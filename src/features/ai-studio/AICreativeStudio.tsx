      ...
      <div className="bg-card-bg border border-card-border rounded-lg shadow-3xl w-full max-w-4xl max-h-[90vh] flex flex-col animate-in zoom-in-95 duration-200 overflow-hidden relative">
        ...
        <div className="px-8 py-6 border-b border-card-border flex justify-between items-center bg-card-bg/50 relative z-10">
          <div className="flex items-center space-x-4">
            ...
            <div>
               <h3 className="text-lg font-bold text-[#111827] dark:text-gray-100  leading-none">{title}</h3>
               <p className="text-[9px] font-bold text-[#6B7280] dark:text-gray-400 uppercase tracking-[0.2em] mt-2">Strategic Node Engineering</p>
            </div>
          </div>
          <button onClick={onClose} className="p-2 text-[#6B7280] dark:text-gray-400 hover:text-[#111827] dark:text-gray-100 transition-all"><X size={24} /></button>
        </div>

        {/* Body */}
        <div className="flex-1 overflow-y-auto p-10 space-y-8 bg-app-bg/50 custom-scrollbar relative z-10">
          <div className="bg-card-bg border border-card-border p-6 rounded-lg text-[#6B7280] dark:text-gray-400 flex items-start shadow-sm border-l border-l-emerald-500/40">
            <Info className="w-6 h-6 mr-4 flex-shrink-0 text-emerald-500" />
            <div className="flex-1">
              <p className="text-[9px] font-bold text-[#6B7280] dark:text-gray-400 uppercase mb-1">{t('aiGeneration')}</p>
          ...
          <div className="space-y-6">
            <div className="flex justify-between items-end bg-card-bg/50 p-6 rounded-t-2xl border border-card-border shadow-sm">
              <div className="space-y-1">
                <label className="text-[8px] font-bold text-[#6B7280] dark:text-gray-400 uppercase block mb-1 ml-1">Context Category</label>
                <div className="text-lg font-bold text-slate-100 ">{post.topic || 'Global Stream'} • {post.phase || 'Strategic Content'}</div>
              </div>
              <div className="flex gap-3">
                <button onClick={() => setShowAdvanced(prev => !prev)} className={`px-5 py-2.5 rounded-xl text-[9px] font-bold uppercase transition-all flex items-center border ${showAdvanced ? 'bg-indigo-500 border-indigo-500 text-white shadow-xl' : 'bg-accent-bg text-[#6B7280] dark:text-gray-400 border-card-border hover:text-[#111827] dark:text-gray-100'}`}>
                  <Sliders size={14} className="mr-2" /> {showAdvanced ? t('cancel') : 'Options'}
                </button>
                <button 
                  onClick={handleRegenText} 
                  disabled={isGenerating} 
                  className="px-5 py-2.5 bg-emerald-500 text-white rounded-xl text-[9px] font-bold uppercase flex items-center gap-2 hover:bg-emerald-600 shadow-xl shadow-emerald-500/10 transition-all disabled:opacity-30 active:scale-95"
                >
            ...
            {showAdvanced ? (
              <div className="p-6 bg-card-bg border-x border-card-border animate-in slide-in-from-top-2 duration-300">
                <label className="block text-[8px] font-bold text-indigo-400 uppercase mb-3 ml-1">Context Injections</label>
                <Textarea 
                  className="w-full bg-app-bg border border-card-border rounded-xl p-5 text-xs font-bold text-[#111827] dark:text-gray-100 outline-none focus:border-emerald-500/50 transition-all shadow-inner resize-none min-h-[80px]" 
                  value={prompt} 
            ...
            <Textarea 
              className="w-full border border-card-border border-t-0 rounded-b-2xl p-8 text-sm font-bold leading-relaxed text-[#111827] dark:text-gray-100 focus:text-slate-100 outline-none bg-card-bg shadow-inner min-h-[300px] resize-none transition-all" 
              value={content} 
          ...
          <div className="space-y-6">
            <div className="flex justify-between items-center bg-card-bg border border-card-border p-5 px-8 rounded-lg">
               <span className="text-[9px] font-bold text-[#6B7280] dark:text-gray-400 uppercase">Visual Assets Pipeline</span>
               <div className="flex items-center gap-3">
                  <button 
                    onClick={handleRegenImage} 
                    disabled={isGeneratingImage} 
                    className="px-4 py-2 bg-accent-bg text-emerald-500 border border-card-border rounded-xl text-[9px] font-bold uppercase hover:bg-emerald-500 hover:text-white transition-all disabled:opacity-30"
                  >
                  ...
                  <button 
                    ...
                    className="px-4 py-2 bg-slate-100 text-black rounded-xl text-[9px] font-bold uppercase hover:bg-emerald-500 hover:text-white transition-all flex items-center gap-2"
                  >
            ...
            <div className="grid grid-cols-4 gap-6">
              {media.map(item => (
                <div key={item.id} className="relative aspect-square rounded-lg overflow-hidden bg-accent-bg border border-card-border group hover:border-emerald-500/50 transition-all duration-500">
                  {item.type === 'video' ? (
                    <div className="w-full h-full bg-slate-900 flex items-center justify-center relative">
                      <Video className="text-[#2563EB] dark:text-blue-400 opacity-40" size={32} />
                      <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all">
                        <button onClick={() => { setVideoResult({ videoId: 'preview', url: item.data, status: 'ready' }); setShowPlayer(true); }} className="nm-flat p-3 rounded-full text-[#2563EB] dark:text-blue-400">
              ...
              {isGeneratingImage ? (
                <div className="aspect-square bg-card-bg rounded-lg animate-pulse flex flex-col items-center justify-center border border-dashed border-card-border">
                   <Loader2 className="animate-spin text-emerald-500 mb-2" size={24} />
                   <p className="text-[8px] font-bold text-[#6B7280] dark:text-gray-400 uppercase">Searching...</p>
                </div>
              ) : null}
              {media.length === 0 && !isGeneratingImage ? (
                <div className="aspect-square bg-card-bg/50 rounded-lg flex flex-col items-center justify-center border border-dashed border-card-border text-slate-800">
        ...
        <div className="px-8 py-6 border-t border-card-border flex justify-end items-center space-x-6 bg-card-bg/50 relative z-10">
            <button onClick={onClose} className="text-[10px] font-bold uppercase text-[#6B7280] dark:text-gray-400 hover:text-[#111827] dark:text-gray-100 tracking-normal transition-colors">Discard changes</button>
            <div className="flex items-center gap-4">
              <button 
                onClick={handleGenerateVideo}
                disabled={isGeneratingVideo || !content}
                className="px-6 py-3.5 border border-[#D1D5DB] dark:border-white/12 rounded-lg text-[#2563EB] dark:text-blue-400 flex items-center gap-3 text-[10px] font-bold uppercase disabled:opacity-30 mr-2"
              >
              ...
              <button 
                ...
                className="px-6 py-3.5 border border-[#D1D5DB] dark:border-white/12 rounded-lg text-[#6B7280] dark:text-gray-400 hover:text-[#111827] dark:text-gray-100 flex items-center gap-3 text-[10px] font-bold uppercase"
              >
                Save Changes
              </button>
              <button 
                onClick={() => onSave({ content, media }, true)} 
                className="px-10 py-3.5 bg-gradient-to-r from-emerald-500 to-teal-600 text-white rounded-xl font-bold text-[11px] uppercase shadow-xl shadow-emerald-500/20 hover:brightness-110 transition-all transform active:scale-95 flex items-center gap-3"
              >
                <Send size={16} />
                Publish Now
              </button>
